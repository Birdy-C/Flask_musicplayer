...
;(function(window) {
	...
	function Turntable(el, options) {
		...
		this.infoElems = {
			...
			lyrics : this.el.querySelector('.player-info > .lyrics--player'),
	...
	function loadTurntableAssets(callback) {
		new AbbeyLoad([{ 
			'room1' : 'static/mp3/room1.mp3',
			'room2' : 'static/mp3/room2.mp3',
			'room3' : 'static/mp3/room3.mp3',
			'noise' : 'static/mp3/noise1.mp3'
		}], function(bufferList) {
			if( typeof callback === 'function' ) {
				callback(bufferList);
			}
	...
	function showExpander(position, callback) {
	    dynamics.css(expanderEl, { opacity: 1, left: position.x, top: position.y, backgroundColor: '#5d5d69', scale: 0 });
